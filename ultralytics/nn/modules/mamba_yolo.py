...
from .block import EdgeGuidedEnhance

__all__ = ("VSSBlock", "EGVSSBlock", "LiteEGVSSBlock", "GateVSSBlock", "SimpleStem", "VisionClueMerge", "XSSBlock")
...
class EGVSSBlock(VSSBlock):
    """
    Edge-guided VSSBlock.

    Adds a lightweight edge/context enhancement module before LSBlock + SS2D while preserving
    tensor shape, residual path, and output channels.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        hidden_dim = self.proj_conv[0].out_channels
        self.edge_enhance = EdgeGuidedEnhance(hidden_dim)

    def forward(self, input: torch.Tensor):
        input = self.proj_conv(input)
        input = input + self.edge_enhance(input)
        X1 = self.lsblock(input)
        x = input + self.drop_path(self.op(self.norm(X1)))
        if self.mlp_branch:
...
class LiteEGVSSBlock(nn.Module):
    """
    Lightweight edge-guided VSS block.

    Only a portion of channels are processed by SS2D, while the remaining channels go through a cheap
    local depthwise branch. This reduces the amount of expensive state-space computation while keeping
    the same input/output tensor shape as VSSBlock.
    """
    ...
    def __init__(
        ...
        min_branch_channels = 16 if hidden_dim >= 32 else max(hidden_dim // 4, 1)
        ssm_channels = int(round(hidden_dim * partial_ratio / 8) * 8)
        ssm_channels = max(min_branch_channels, ssm_channels)
        ssm_channels = min(hidden_dim - min_branch_channels, ssm_channels)
        if ssm_channels <= 0 or ssm_channels >= hidden_dim:
            ssm_channels = hidden_dim // 2
        local_channels = hidden_dim - ssm_channels
        if local_channels <= 0:
            local_channels = 1
            ssm_channels = hidden_dim - 1
        if ssm_channels <= 0 or local_channels <= 0 or (ssm_channels + local_channels) != hidden_dim:
            raise ValueError(
                f"LiteEGVSSBlock channel split is invalid: hidden_dim={hidden_dim}, "
                f"ssm_channels={ssm_channels}, local_channels={local_channels}"
            )
        self.ssm_channels = ssm_channels
        self.local_channels = local_channels

        self.ssm_norm = norm_layer(ssm_channels)
        self.ssm_lsblock = LSBlock(ssm_channels, ssm_channels)
        self.ssm_op = SS2D(
            d_model=ssm_channels,
            d_state=ssm_d_state,
            ssm_ratio=ssm_ratio,
            ssm_rank_ratio=ssm_rank_ratio,
            dt_rank=ssm_dt_rank,
        ...
        self.local_branch = nn.Sequential(
            nn.Conv2d(local_channels, local_channels, kernel_size=3, stride=1, padding=1, groups=local_channels, bias=False),
            nn.BatchNorm2d(local_channels),
            nn.SiLU(),
            nn.Conv2d(local_channels, local_channels, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(local_channels),
            nn.SiLU(),
        )
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = x + self.edge_enhance(x)
        x_ssm, x_local = torch.split(x, [self.ssm_channels, self.local_channels], dim=1)

        x_ssm_res = self.ssm_lsblock(x_ssm)
        x_ssm = x_ssm + self.drop_path(self.ssm_op(self.ssm_norm(x_ssm_res)))
        x_local = x_local + self.local_branch(x_local)

        x = self.fuse(torch.cat((x_ssm, x_local), dim=1))
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x


class GateVSSBlock(nn.Module):
    """
    Gate-controlled VSS block.

    The block keeps the original VSS input/output contract, but replaces the fixed residual fusion with a
    learnable balance between a local convolution branch and the SS2D branch. This lets the model adaptively
    choose how much local structure or long-range context to use at each channel.
    """
    ...
    def __init__(
        ...
        gate_hidden = max(hidden_dim // gate_reduction, 16)
        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(hidden_dim, gate_hidden, kernel_size=1, stride=1, padding=0, bias=False),
            nn.SiLU(),
            nn.Conv2d(gate_hidden, hidden_dim * 2, kernel_size=1, stride=1, padding=0, bias=True),
        )
    ...
    def forward(self, input: torch.Tensor):
        ...
        ssm_input = self.lsblock(x)
        ssm_delta = self.op(self.norm(ssm_input))
        local_delta = self.local_branch(x)

        gate = self.gate(x)
        b, _, _, _ = gate.shape
        gate = gate.view(b, 2, x.shape[1], 1, 1).softmax(dim=1)
        fused_delta = gate[:, 0] * ssm_delta + gate[:, 1] * local_delta

        x = x + self.drop_path(fused_delta)
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
